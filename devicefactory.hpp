...
public:
    ...
    virtual std::shared_ptr<DeviceHandle> format(const std::string& path, const std::vector<uint8_t>& dev_id) const
    {
    ...
    virtual std::shared_ptr<DeviceHandle> open(const std::string& path, bool require_signature = false, int mode = O_RDONLY) const
    {
